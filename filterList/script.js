...
// an empty array which will hold the cities
const cities = [];

// fetching the data from API, which will return a promise
fetch(endpoint)
...
// a function to filter the array when something is typed in the search box
...
// caching the search box and suggestions to variables
const searchInput = document.querySelector('.search');
const suggestions = document.querySelector('.suggestions');

// a function to add separating commas to numbers
function numberWithCommas(x) {
  return x.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',');
}

// a display function that will be called whenever the value in the search box is changed
